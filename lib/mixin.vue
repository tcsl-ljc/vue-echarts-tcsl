...
export default {
  ...
  methods: {
    initChart() {
      try {
        const dcChart = echarts.init(
          document.getElementById(this.specialId),
          null,
          { renderer: "canvas" }
        );
        ...
        console.trace(`'${this.specialId}' dom unRendered`);
      }
    },
    // tcsl内部调用使用异步加载主题文件
    ...
    resize() {
      try {
        echarts
          .getInstanceByDom(document.getElementById(this.specialId))
          .resize();
      } catch (e) {
        console.trace(`'${this.specialId}' dom unRendered`);
      }
